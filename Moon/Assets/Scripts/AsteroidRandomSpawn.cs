...
public class AsteroidRandomSpawn : MonoBehaviour {
	...
	void CreateAsteroid(){
		...
		// Asteroid is made a chile of the AsteroidSpawner object to keep the hierarchy view tidy.
		// Asteroid is given a random size between 5 and 20, upon creation.
		...
		cloneAsteroid.transform.parent = GameObject.FindGameObjectWithTag("AsteroidSpawner").transform;
		cloneAsteroid.transform.localScale = Vector3.one * Random.Range (5, 20);
	}
	...
	void Update () {
		...
		// If an asteroid goes out of bounds, it is destroyed -- otherwise it is destroyed after secondsUntilDestroy.
